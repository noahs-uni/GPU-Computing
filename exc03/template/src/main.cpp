...
extern void globalMemCoalescedKernel_Wrapper(dim3 gridDim, dim3 blockDim, int *dmem1, int *dmem2, size_t dataSize);
extern void globalMemStrideKernel_Wrapper(dim3 gridDim, dim3 blockDim, int *dmem1, int *dmem2, int stride);
extern void globalMemOffsetKernel_Wrapper(dim3 gridDim, dim3 blockDim, int *dmem1, int *dmem2, int offset);
...
main ( int argc, char * argv[] )
{
    ...
    if ( chCommandLineGetBool("global-stride", argc, argv) ) {
        ...
        // determine memory size from kernel launch parameters and offset
        // Need space for (gridSize * blockSize) elements plus offset elements
