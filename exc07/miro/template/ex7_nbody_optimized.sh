...
# Output file for optimized n-body results
OUTPUT_FILE="nbody_optimized_benchmark_results.txt"
...
for num_elements in "${OBJECT_COUNTS[@]}"; do
    # Run optimized nbody with --silent flag to suppress iteration output
    OUTPUT=$(./bin/nbody_optimized --silent -p -s $num_elements -i $NUM_ITERATIONS -t $THREADS_PER_BLOCK 2>&1 | sed 's/\x1b\[[0-9;]*m//g')
