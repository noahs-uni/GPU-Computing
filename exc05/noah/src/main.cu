...
matMul_Kernel(int matrixSize, float* matrixA, float* matrixB, float* matrixC)
{
    ...
    if (elementIdx < matrixSize && 
        ...
        for (int k = 0; k < matrixSize; k++) {
            value += matrixA[elementIdy * matrixSize + k] * matrixB[k * matrixSize + elementIdx];
        }
...
shMatMul_Kernel(int matrixSize, float* matrixA, float* matrixB, float* matrixC)
{
    ...
    float *sh_MatrixB = &(sh_Mem[1 * blockDim.x * blockDim.y]);
    ...
    if (col < matrixSize && 
        row < matrixSize) {
        for (int m = 0; m < matrixSize / blockDim.x; m++) {
            sh_MatrixA[threadIdx.y * blockDim.x + threadIdx.x] = 
                matrixA[row * matrixSize + (m * blockDim.x + threadIdx.x)];
            sh_MatrixB[threadIdx.y * blockDim.x + threadIdx.x] = 
                matrixB[col + (m * blockDim.x + threadIdx.y) * matrixSize];
            __syncthreads();
            for (int k = 0; k < blockDim.x; k++) {
                value += sh_MatrixA[threadIdx.y * blockDim.x + k] * 
                         sh_MatrixB[k * blockDim.x + threadIdx.x];
            }
            __syncthreads();
        }
        matrixC[elementId] = value;
...
main(int argc, char * argv[])
{
    ...
    if (!dontCheckResult) {
        dontCheckResult = chCommandLineGetBool("no-checking", argc, argv);
    }
