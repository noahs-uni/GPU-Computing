...
matMul_Kernel(int matrixSize, float* matrixA, float* matrixB, float* matrixC)
{
    ...
    if (elementIdx < matrixSize && 
        ...
        {
            Aelement = matrixA[elementIdy * matrixSize + i];
            Belement = matrixB[i * matrixSize + elementIdx];
            value += Aelement * Belement;
        }
        matrixC[elementId] = value;
    }
}
...
main(int argc, char * argv[])
{
    ...
    bool host = chCommandLineGetBool("h", argc, argv);
    if (!host) {
        host = chCommandLineGetBool("host", argc, argv);
    }
    ...
    if (host) {
        // Host-only execution: allocate, init, run blocked CPU matrix multiply and print results.
        float* h_matrixA = static_cast<float*>(malloc(static_cast<size_t>(matrixSize * sizeof(*h_matrixA))));
        float* h_matrixB = static_cast<float*>(malloc(static_cast<size_t>(matrixSize * sizeof(*h_matrixB))));
        float* h_matrixC = static_cast<float*>(calloc(static_cast<size_t>(matrixSize), sizeof(*h_matrixC)));
        ...
        // Run blocked host matrix multiplication (use block size 32 as in other call sites)
        kernelTimer.start();
        MatrixMulOnHostBlocked(h_matrixA, h_matrixB, h_matrixC, static_cast<long>(matrixWidth), 32);
        kernelTimer.stop();

        if (verbose) {
            std::cout << "***" << std::endl
                        << "*** Results (Host):" << std::endl
                        << "***    Matrix Size: " << matrixSize << std::endl
                        << "***    Time for Matrix Multiplication: " << 1e3 * kernelTimer.getTime()
                        << " ms" << std::endl
                        << "***" << std::endl;
        } else {
            std::cout << 1e3 * kernelTimer.getTime() << std::endl;
        }
    ...
    if (verbose) {
        ...
        std::cout << grid_dim.x << "x" << grid_dim.y << "x" << grid_dim.z 
                        << ","
                << block_dim.x << "x" << block_dim.y << "x" << block_dim.z
                        << ",";
    }
    ...
    if(verbose) {
        ...
        std::cout << 1e3 * memCpyH2DTimer.getTime() << ","
                  << 1e-9 * memCpyH2DTimer.getBandwidth(2 * matrixSize * sizeof(*h_matrixA)) << ","
...
printHelp(char * programName)
{
    std::cout << "Help:" << std::endl
              ...
              << "" << std::endl
              << "  --shared" << std::endl
              << "  Use shared matMul" << std::endl
              << "" << std::endl
              << "  --verbose" << std::endl
              << "  Use pretty output (non csv)" << std::endl
              << std::endl;
}
